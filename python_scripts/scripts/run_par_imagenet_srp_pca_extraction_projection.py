...
# e.g. to call it:
# mpiexec -np 4 run_par_imagenet_srp_pca_extraction_projection.py --model_name='alexnet' --pkg=torchvision --dataset_name=imagenet_val --PCs_dataset=imagenet_val --pooling=all --n_srp_components=1000 --n_pca_components=10 --batch_size=100 --img_size=224

parser = argparse.ArgumentParser(
    description="Extract ImageNet validation activations and project them into a dataset-specific SRP-to-PCA space."
)
parser.add_argument("--model_name", required=True,)
parser.add_argument("--pkg", required=True)
parser.add_argument("--dataset_name", default="imagenet_val", help="Name used in the saved projected feature filename.")
